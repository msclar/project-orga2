...
Comenzaremos explicando la paralelización de un paso del método de Laplace, pues es considerablemente más sencillo que el método de Jacobi y su error.

\begin{lstlisting}
void laplaceStep_C(double* res, double* phi, int max_i, int max_j) {
	int i, j;
...
donde 
\begin{centering}
\texttt{ymm0 = [phi(i-1, j) | phi(i-1, j+1) | phi(i-1, j+2) | phi(i-1, j+3)]}, \\
\texttt{ymm1 = [phi(i, j-1) | phi(i, j) | phi(i, j+1) | phi(i, j+2)]}, \\
\texttt{ymm2 = [phi(i, j+1) | phi(i, j+2) | phi(i, j+3) | phi(i, j+4)]}, \\
\texttt{ymm3 = [phi(i+1, j) | phi(i+1, j+1) | phi(i+1, j+2) | phi(i+1, j+3)]} \\
\texttt{suma = [phi(i-1, j) + phi(i, j-1) + phi(i, j+1) + phi(i+1, j) | \\
                phi(i-1, j+1) + phi(i, j) + phi(i, j+2) + phi(i+1, j+1) | \\
                phi(i-1, j+2) + phi(i, j+1) + phi(i, j+3) + phi(i+1, j+2) | \\
                phi(i-1, j+3) + phi(i, j+2) + phi(i, j+4) + phi(i+1, j+3)]}
\end{centering}

En realidad, la implementación evita utilizar tantos registros YMM utilizando un acumulador que comienza en cero y al que se suma cada registro. La explicación se hizo con cuatro registros simultáneos para mayor claridad.
